...
type Transaction struct {
	...
	TimeStamp                 string `json:"timestamp,omitempty"`
	Error                     string
	BaseFeePerGas             *gokit.HexBig     `json:"baseFeePerGas,omitempty"`
	TotalMaxPriorityFeePerGas *gokit.HexBig     `json:"totalMaxPriorityFeePerGas,omitempty"`
	MaxPriorityFeePerGas      *gokit.HexBig     `json:"maxPriorityFeePerGas,omitempty"`
	MaxFeePerGas              *gokit.HexBig     `json:"maxFeePerGas,omitempty"`
	ActualCost                *gokit.HexBig     `json:"actualCost,omitempty"`
	ActualGas                 gokit.HexUint64   `json:"actualGas,omitempty"`
	Logs                      []*gokit.EventLog `json:"logs,omitempty"`

	// helper fields to speed up parsing: will not insert into DB.
	CtType gokit.ContractType
}
...
func IsNFTByClient(tokenAddr string) int {
	err, ttype := gec.CheckNFTType(tokenAddr)
	if err != nil {
		fmt.Println(err)
	}
...
func Demo() {
	res, err := gokit.HexStringToHexBig("0x706e42f8887fabd037b5cbd65ecae40e823900254f9dae72c8ff4bde33e62b1d")
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println(res)
